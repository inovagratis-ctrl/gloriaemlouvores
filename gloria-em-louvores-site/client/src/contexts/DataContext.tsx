...
const defaultVideos: Video[] = [
  { id: '1', youtubeId: 'MF8p-aj16Zg', title: 'O SOM DO MILAGRE | 12 Louvores Para Quem Precisa de um Milagre', views: '5.6K views', category: 'adoracao' },
  { id: '2', youtubeId: 'TvNd8KgQRGc', title: 'TRONO DE DEUS | 70 Louvores Para Afastar Todo Mal', views: 'Louvores poderosos', category: 'protecao' },
  { id: '3', youtubeId: 'vD6vp5AJ4bU', title: 'Louvor Gospel Worship Emocionante 2026', views: 'Worship poderoso', category: 'adoracao' },
];
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    localStorage.setItem('gloria-videos', JSON.stringify(videos));
  }, [videos]);

  useEffect(() => {
    localStorage.setItem('gloria-blog', JSON.stringify(blogPosts));
  }, [blogPosts]);

  useEffect(() => {
    localStorage.setItem('gloria-requests', JSON.stringify(prayerRequests));
  }, [prayerRequests]);
  ...
  const addPrayerRequest = (request: Omit<PrayerRequest, 'id' | 'date' | 'read'>) => {
    setPrayerRequests(prev => [...prev, { ...request, id: Date.now().toString(), date: new Date().toLocaleDateString('pt-BR'), read: false }]);
  };
  ...
  return (
    <DataContext.Provider value={{ videos, blogPosts, prayerRequests, addVideo, updateVideo, deleteVideo, addBlogPost, updateBlogPost, deleteBlogPost, addPrayerRequest, markAsRead, deletePrayerRequest }}>
      {children}
